...
\end{eqnarray}
In jedem Fall erscheint ein Faktor von $\omega$ aufgrund der Photonenenergie $\hbar \omega$, da die rechte Seite einzelne Absorptions- oder Emissionsereignisse betrachtet, aber die linke Seite  Spektren in Form von Leistung (nicht Photonen) pro Spektralintervall bezeichnet. Das Fluoreszenzspektrum erhält einen zusätzlichen Faktor von $\omega^2$ aufgrund der optischen Modendichte im dreidimensionalen Raum, wie es im Schwarzkörperspektrum und in der  Beziehung zwischen den Einstein-Koeffizienten $A$ und $B$ auftritt.
